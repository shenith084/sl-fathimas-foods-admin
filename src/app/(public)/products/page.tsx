...
function ProductsContent() {
  ...
  return (
    <div className="bg-[#FAF7F2] min-h-screen relative pb-12">
      {/* Toast notifications */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 bg-[#556B4F] text-white px-5 py-3 rounded-xl shadow-lg flex items-center gap-2 animate-slide-up transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <button
              ...
              className="flex items-center gap-2 border border-gray-200 text-[#444] text-sm font-medium px-4 py-2 rounded-xl hover:border-[#556B4F] hover:text-[#556B4F] transition-colors lg:hidden"
            >
                  ...
                  {["Popular", "Price: Low to High", "Price: High to Low", "Newest"].map((s) => (
                    <li key={s}>
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                          sortOption === s
                            ? "bg-[#556B4F] text-white font-semibold"
                            : "text-[#555] hover:bg-[#FAF7F2] hover:text-[#D98C1F]"
                        }`}
                      >
                        {s}
                ...
                {sortedProducts.map((product) => (
                  ...
                  >
                    <Link href={`/products/${product.slug}`} className="block">
                      {/* Image area */}
                      <div className="relative bg-gradient-to-br from-[#F4EFE6] to-[#FAF7F2] h-40 flex items-center justify-center">
                        {product.badge && (
                    ...
                    <div className="p-3 pt-0 flex items-center justify-between mt-auto">
                      ...
                      <button
                        ...
                        className="w-8 h-8 bg-[#556B4F] hover:bg-[#D98C1F] text-white rounded-lg flex items-center justify-center transition-colors duration-200 shadow-sm"
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="bg-[#FAF7F2] min-h-screen flex items-center justify-center text-[#556B4F] font-semibold">Loading catalog...</div>}>
      <ProductsContent />
    </Suspense>
  );
}
